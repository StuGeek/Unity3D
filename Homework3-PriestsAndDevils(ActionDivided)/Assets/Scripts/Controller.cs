...
namespace PriestsAndDevils {
	...
	public class BoatController {
		...
		private ActionController actionController; // 行为控制器
		...
		public BoatController() {
			...
			// 添加行为控制器
			actionController = boatView.AddActionScript();
		}
		...
		// 获取船所在的岸边
		public int GetBoatSide() {
			return actionController.GetBoatSide();
		}
		...
		public Vector3 GetFreePosition() {
			...
			int side = actionController.GetBoatSide();
	...
	public class RoleController {
		...
		public RoleController(string roleName, int number) {
			...
			// 添加行为控制器
			actionController = roleView.AddActionScript();
			actionController.SetClickRole(this);
		}

		public void InitRoleController(ShoreController rightShore) {
			// 初始化行为控制器
			actionController.InitActionController();
			// 将角色放到右岸上
			PutRoleOnShore(rightShore);
			SetRolePos(rightShore.GetFreePosition());
		}
		...
		public void MoveToPosition(Vector3 endPosination) {
			actionController.SetMovePos(endPosination);
		}
	...
	public class MainController : MonoBehaviour {
		...
		private JudgeController judge; // 裁判控制类
		...
		public void InitGame() {
			...
			for (int i = 0; i < roles.Length; i++) {
				roles[i].InitRoleController(rightShore);
				// 右岸控制器中关于角色的信息也被初始化
				rightShore.PutRoleOnShore(roles[i].GetRoleViewName());
			}
			judge.InitJudgeController();
		...
		public void MoveBoat() {
			// 如果游戏状态不是正在进行或船上没有角色，那么不移动
			if (!judge.isPlayingGame() || judge.IsNoRoleOnBoat(boat)) {
				return;
			} else {
				// 否则直接将船开到另一边
				boat.GetActionController().MoveToOtherSide();
			}
			// 检查游戏状态
			int gameState = judge.CheckGameState(leftShore, rightShore, boat);
			// 显示相应提示
